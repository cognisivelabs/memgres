...
/**
 * Test class for INTERSECT and EXCEPT set operations (basic functionality)
 */
class IntersectExceptBasicTest {
    ...
    void testSimpleIntersect() throws Exception {
        ...
        // Verify the intersection values (convert to set for order-independent comparison)
        Set<Integer> actualValues = new HashSet<>();
        for (Row row : rows) {
            actualValues.add(((Number) row.getData()[0]).intValue());
        }
        
        Set<Integer> expectedValues = Set.of(3, 4, 5);
    ...
    void testSimpleExcept() throws Exception {
        ...
        Set<Integer> expectedValues = Set.of(1, 2);
        assertEquals(expectedValues, actualValues);
    }
    ...
    void testIntersectWithMultipleColumns() throws Exception {
        ...
        // Verify the common employees/contractors (order-independent)
        Set<String> actualNames = new HashSet<>();
        for (Row row : rows) {
            actualNames.add((String) row.getData()[1]);
        }
        
        Set<String> expectedNames = Set.of("Alice", "Carol");
    ...
    void testExceptWithMultipleColumns() throws Exception {
        ...
        Set<String> expectedNames = Set.of("Bob", "David", "Eve");
        assertEquals(expectedNames, actualNames);
    }
    ...
    void testExceptNoDuplicates() throws Exception {
        ...
        Set<Integer> expectedValues = Set.of(1, 2, 8, 9);
        assertEquals(expectedValues, actualValues);
    }
